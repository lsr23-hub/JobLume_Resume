 ...
 * 经历类别的封闭词表。
 *
 * **为什么是混合词表（行业 + 内容类型）**：类别唯一的用途是驱动
 * `rankGeneric` 的「同类经历衰减」——它要回答的问题是「哪些条目在抢同一个位置」。
 * 而一半的条目（荣誉、语言、校园、学历）根本没有「行业」。
 *
 * 这不是猜的，是实测的：只给工作/项目打行业标签时，**没打标签的条目因不参与衰减
 * 反而稳稳挤在前面**，top-8 的类别数只有 2、顺序几乎没变；改成混合词表后
 * top-8 的类别数升到 4，金融项目上移到第 5、日语降到第 8。
 *
