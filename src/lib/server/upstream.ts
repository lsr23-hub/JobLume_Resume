 ...
 * 调上游 LLM 时的中断控制。
 *
 * 上游卡住时，裸 `fetch` 会让连接无限期挂着。这点用一个只收请求、永不响应的
 * tarpit 服务实测过 —— 客户端 25 秒放弃之后，服务端仍在等，直到上游被强杀
 * 才报 `SocketError: other side closed`。
 *
 * 另外，浏览器取消请求也**不会**自动传到上游：用户关掉页面，服务端照样把那次
 * 生成跑完，token 照烧。所以这里把两个信号合并：超时是一道闸，客户端断开是
 * 另一道。
 */
