 ...
 *
 * **当前模型（S1）**：真相源是浏览器 localStorage，这里只做单向镜像 —— 磁盘上
 * 手改过的内容会被下一次同步覆盖。
 *
 * ⚠️ 但这**不是目标形态**。`plan/saves-design.md` 定的是「磁盘为权威副本 + 按记录
 * 内容哈希对账」：届时手改文件会生效、只在两边都改时才叫冲突（S3/S4 实施）。
 * 所以这里的单向覆盖是**待替换的行为**，不是设计终点。
 */
