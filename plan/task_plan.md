...
| 3b | 删除用户（二次确认 + 连带清理档案/简历/分析） | ✅ 决策 4 收尾 |
...
| C4 | e2e 种子同时写盘 | ✅ `6effc08` |
| C4b | **写前备份 `.bak`**（单副本保险，见下） | ✅ 见下 |
| C5 | 启动从磁盘读回（真相源翻转） | ⬜ |
| C6 | 三个 store 不再持久化 | ⬜ |
...
> ⚠️ **C5 及之后已被取代**（2026-09-20）。方案改为 `plan/saves-design.md`：
> **保留三个 store 的 persist**（浏览器仍是工作副本），改成「手动保存 + 五个明确写盘时机
> + 按内容哈希对账」；`journal` 与 `legacy` 两个模块**已删除**（从未接线，且新模型下不需要）。
> C1–C4b 的成果（退役本地文件夹同步、读取端点、写前备份 `.bak`、e2e 种子写盘）**全部保留**，
> 它是新方案的地基。本表不再作为路线图使用。
...
原计划写的是「8 个脚本的种子改成照旧写 localStorage 再写一份盘」。实际做的时候发现
**写盘与导航之间不能有等待** —— 镜像的防抖 flush（1500ms）会把内存里的状态写回磁盘，
可能盖掉刚写下的种子。所以 `seedSaves(page)` 被放在紧挨着导航的前一条语句，中间不插
`waitForTimeout`。翻转那一步（C5）如果出现「种子没生效」的怪现象，先查这里。

### ⚠️ 本轮我犯的一个错（如实记录）

验证 `seedSaves` 是否真的写盘时，我临时停用了 e2e 的清扫钩子，跑完用
`rm -rf saves/*/` 收尾 —— 那条命令把**开发者真实的那份镜像目录也删了**
（`saves/876a6319-…/`，档案里是一个真实姓名）。

**数据本身没丢**：当前提交下 `persist` 仍在，`saves/` 是派生副本，真相源是浏览器的
localStorage，而我没有碰过浏览器。下一次打开应用，镜像会把 `saves/<uid>/profile.json`
重新写出来（就是本次会话后面要做的 C5 之后，它还会成为唯一副本 —— 那才是真正危险的
状态）。

**教训**：`saves/` 下**永远不要用 glob 删除**。清理只能走 `userScope.mjs` 里那套
快照式清扫（它只删本次运行新建的目录）。这一条也写进了记忆。

### C4b：写前备份（因为上面那次事故加的）

`saves/` 成为唯一副本之后，「写进去的内容是坏的」没有第二处兜底 —— 一次写出空档案
就真没了。所以 `writeSaveFile` 在覆盖之前先把原文件复制成 `X.json.bak`，只留一代。

边界说清楚：**`.bak` 保护的是「写坏」，不是「反悔」。** `removeSaveFile` 会把
`.json` 与 `.bak` 一起清掉 —— 删除是用户的显式动作，留一份「删了还在」的副本正是
