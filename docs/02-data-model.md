...
**磁盘副本（D36，取代 D35）**：三份状态**以磁盘上的 `saves/<userId>/` 为权威副本**
（`profile.json` / `resumes/<id>.json` / `jds/<id>.json` / `images/<id>.<ext>`），
浏览器里的 localStorage 是**工作副本**。写盘发生在**五个明确时机**上（点「立即保存」/
切用户 / 关编辑器 / 页面隐藏或关闭 / 应用内离开时确认），**启动时读回并对账** ——
磁盘上被手改过的内容会生效；两边都改过则弹对话框逐条问，不自动覆盖任何一边。
决策与取舍见 **D36**，模块分层见 README「项目结构」里 `lib/saves/` 那张表，
...
**问题**：上游把照片（`basic.photo`）和证书（`Certificate.url`）存为 Base64 字符串，
放在 localStorage。配额约 5MB，2-3 张证书图片即可撑满。

**方案**：二进制**从数据里搬出去**，数据里只留一个文件名引用：

```
saves/<userId>/images/<imageId>.<ext>   权威副本（原始字节）
IndexedDB: JobLumeImageDB               缓存（索引不到就从磁盘拉）
数据里的引用: img_<uuid>.<ext>            就是那个文件名
```

D5 当初只把它从 localStorage 挪进 IndexedDB，于是图片仍然只在**这一个浏览器**里 ——
备份不含它、换台机器看不到、清掉站点数据就没了。D36 把它挪到磁盘上，IndexedDB 降级为缓存。

- **扩展名由服务端从请求的 `Content-Type` 推出来，客户端不传** —— 存下来的路径永远由
  服务端拼，客户端左右不了它。白名单只收 jpeg/png/webp/gif/avif（canvas 能编出来的），
  SVG 不在内（它过一遍 canvas 出来就是 PNG）
- **选中即上传**，不等手动保存：图片是素材不是文档、没有「草稿」语义；延后上传会造出
  「数据已保存但照片没保存」这种没法向用户解释的中间态
- **引用强制以 `img_` 开头**：少了这条，任意一个长得像文件名的字符串（`/avatar.png`
  这类静态路径）都会被当成引用去查
- **渲染前解析成 `blob:` URL**（在 `optimizeImages` / `bakeObjectFitCoverImages`
  之前完成）。字节是**后到**的（缓存没有时从磁盘拉），而 effect 的 deps 只看「引用变了
  没有」—— 所以有一个 `imageEpoch` 计数器进 deps
- **孤儿回收**：保存成功后服务端扫一遍引用，删掉 `images/` 里没人用的文件。带**一小时
  宽限期** —— 刚上传还没保存的图片，在磁盘的数据看来也是孤儿
- **兼容**：旧的 `idb:img_xxx` 引用照常解析（那批图只在 IndexedDB 里）；外链、`data:`、
  静态路径一律原样透传，所以老数据不需要迁移
...
| **D36** | **`saves/<userId>/` 是权威副本，浏览器是工作副本**（取代 D35）：写盘发生在**五个明确时机**（点「立即保存」/ 切用户 / 关编辑器 / 页面隐藏或关闭 / 应用内离开时确认），不再是防抖自动镜像；**启动时读回并对账**，磁盘上被手改过的内容生效。对账用「每条记录的**内容哈希** + 磁盘上的基线」做三方比对（本地 / 磁盘 / 基线）；两边都改过则逐条问用户，**不自动覆盖任何一边**。图片二进制同样落盘（`images/<imageId>.<ext>`），IndexedDB 从「存储」降级为「缓存」 | 维持 D35 的单向镜像 / 用时间戳做双向合并 / 浏览器不留数据（磁盘唯一真相源） | 三条否决理由：① **时间戳不可靠** —— 三个 store 的 `updatedAt` 都会被**视图操作** bump（切板块标签、拖项目顺序都会让简历「变新」），按它判断谁更新会静默覆盖真实编辑；而内容哈希只回答「这条比上次同步时变了没有」，不依赖时钟、也不需要合并算法。② **浏览器不留数据会让没有服务端的部署彻底不可用**（Cloudflare Workers 没有 `node:fs`，静态托管连端点都没有），而保留 localStorage 时读盘失败只降级成「纯浏览器模式」。③ 单向镜像挡住了产品要的心智模型「文件夹就是我的数据」—— 清缓存不丢、手改文件生效、换机器可用，这三件事单向一件都做不到。**已知边界**：① 两边都改的冲突要用户逐条选（刻意不做自动合并）②「本地没有 + 磁盘有 + 基线认」分不清「缓存被清」与「删了但那次保存没成功」，选**恢复**（清缓存不丢是头等目标，「删除被撤销」至多多按一次删除）③ 多标签页后写覆盖先写（未处理）。**D35 那条「删用户不删目录」的边界随之消失** —— 现在删用户会连带删目录 |
| D35（已废止，保留原文） | ~~**`saves/<userId>/` 是单向镜像**：浏览器 localStorage 仍是真相源，改动防抖 1.5s 写一份到磁盘；磁盘上改的不会被读回来 | 用 saves/ 当真相源（双向同步） | 双向同步要处理冲突合并，而这是单人本地工具，收益远小于复杂度。单向的话最坏情况只是「镜像落后」或「手改被覆盖」，不会丢数据。**已知边界**：删用户不会删掉 `saves/` 下对应目录 —— 服务端只做单文件读写，递归删除的破坏面比写文件大得多 |
