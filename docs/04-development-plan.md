...
- [ ] 能生成通用简历并导出 PDF —— 生成与渲染已端到端实测通过（4 套模板全部渲染出项目经历）；**导出本身有一处已知缺陷**，见下
  - **打印导出的 PDF，文字层里有一部分汉字是康熙部首**：实测一份简历 160 个汉字中 18 个（11.3%）落在 U+2E80–U+2FDF，例如 `立→⽴`、`大→⼤`、`工→⼯`、`页→⻚`。
    字形显示正常，但**复制粘贴与 ATS 解析会拿到错字**，而导出弹窗里写的正是「文字可选中、可被 ATS 解析」。
  - 根因不在本项目：Chromium 生成 ToUnicode CMap 时，对「汉字与康熙部首共用同一字形」的字符挑了部首码位。
    最小复现（同一段文字，项目自带字体与系统 PingFang SC 各一行）**两种字体都复现**，与字体选择无关；
    这是 CJK 领域的常见问题（社区有专门的 Kangxi Radical Fixer 扩展，Apache PDFBox 也有对应 issue）。
  - **消费侧有救**：康熙部首多数带兼容分解，`NFKC` 归一化能还原（`⽴→立`、`⼤→大`…）。
    唯一还原不了的是 `⻚`（U+2EDA），它没有兼容分解。会做归一化的解析器读得对，裸解析的读不到。
  - **生产侧暂时无解**：CMap 由打印引擎写出，应用层改不了。另一条导出路径（长页 PDF）走 canvas 位图，
    视觉无损但**完全没有文字层**。两条路是「11% 的字读错」与「全部读不到」之间的取舍。
  - 状态：**未修，但已确认可修。** 原先判「非本项目可修」是基于「CMap 由 Chromium 打印引擎写出」——
    这对**当前实现**成立，但项目已有 `jspdf` 这条自绘路径。
    2026-09-19 做过 A/B 对照（同一段文字，见 `plan/notes.md` §七）：Chromium 打印 **11/20 个汉字**成康熙部首，
    **jsPDF 自绘 0 个**（它按输入码位写 ToUnicode，不做「按字形反查」，因此不踩这个坑）。
  - 走自绘的两个条件：**① jsPDF 只支持 TrueType**，项目 10 个 `@font-face` 里 6 个是 OTF/CFF，不可用
    （NotoSansSC ×3、SourceHanSerifSC ×3，需补 TTF 版同款字体）；
    **② 4 套模板的版面复刻未验证** —— 这才是工作量的大头。
  - 在方案定下来之前，**导出弹窗里「文字可选中、可被 ATS 解析」是过度承诺，应先改成准确说法**。
